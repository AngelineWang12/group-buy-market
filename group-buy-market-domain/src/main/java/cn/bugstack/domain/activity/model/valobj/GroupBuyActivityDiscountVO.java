...
public class GroupBuyActivityDiscountVO {
     ...
     * 折扣配置
     */
    private List<DiscountItem> items;   // 多个优惠项（排兵布阵时使用）

    @Data
    public static class DiscountItem {
        private String plan;            // 策略标识：DIRECT_REDUCE / DISCOUNT / FULL_REDUCE / COUPON ...
        private Integer priority;       // 优先级（越小越先）
        private boolean combinable;     // 是否允许与其它叠加（常规 true；如“券”常为 false）
        private String mutexGroup;      // 互斥组（同组只保留一个）
        private BigDecimal threshold;   // 门槛（满多少可用）
        private BigDecimal cap;         // 单项封顶（最多减多少）
        private Map<String, Object> args; // 策略参数（如折扣率、满减档位等）
    }
    /**
     * 拼团方式（0自动成团、1达成目标拼团）
     */
